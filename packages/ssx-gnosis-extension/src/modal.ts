...
const getErrorModal = (): Element => {
  ...
  use.setAttributeNS('http://www.w3.org/1999/xlink', "xlink:href", "#image0_36_1642");
  ...
  const svgGnosis = document.createElementNS("http://www.w3.org/2000/svg", "svg");
...
const getBaseModal = (): Element => {
  ...
  gLogo.setAttribute("transform", "translate(0.000000,400.000000) scale(0.100000,-0.100000)");
  gLogo.setAttribute("fill", "#ffffff");
  gLogo.setAttribute("stroke", "none");
  const pathLogo1 = document.createElementNS("http://www.w3.org/2000/svg", "path");
  pathLogo1.setAttribute("d", "M1331 3248 c-70 -36 -81 -67 -81 -231 0 -125 -2 -143 -22 -178 -36 -65 -79 -79 -239 -79 -160 0 -191 -11 -226 -80 -22 -42 -23 -54 -23 -295 0 -225 2 -254 20 -292 34 -76 57 -83 270 -83 166 0 188 2 217 20 63 38 67 58 73 330 6 272 8 282 73 317 29 17 78 18 615 20 661 2 627 -2 670 83 20 40 22 58 22 200 0 142 -2 160 -22 200 -13 24 -36 53 -51 64 -28 21 -41 21 -640 24 l-612 2 -44 -22z");
  const pathLogo2 = document.createElementNS("http://www.w3.org/2000/svg", "path");
  pathLogo2.setAttribute("d", "M1815 2286 c-41 -18 -83 -69 -90 -109 -4 -18 -5 -105 -3 -194 3 -176 7 -189 72 -237 25 -19 41 -21 209 -21 208 0 233 7 271 80 19 36 21 57 21 210 0 160 -1 172 -23 207 -41 66 -84 78 -268 78 -109 -1 -168 -5 -189 -14z");
  const pathLogo3 = document.createElementNS("http://www.w3.org/2000/svg", "path");
  pathLogo3.setAttribute("d", "M2770 1991 c-19 -10 -45 -36 -57 -57 -22 -37 -23 -49 -23 -275 0 -228 -1 -237 -22 -269 -13 -18 -39 -41 -58 -51 -33 -18 -69 -19 -608 -19 -559 0 -574 -1 -613 -21 -67 -34 -79 -73 -79 -264 0 -147 2 -165 23 -205 41 -84 9 -80 702 -80 693 0 659 -4 703 80 20 40 22 59 22 185 0 121 3 144 20 172 32 53 59 60 225 65 168 4 201 14 235 71 19 30 20 52 20 311 l0 279 -26 34 c-44 58 -64 63 -256 63 -150 0 -178 -3 -208 -19z");
  ...
  pathClose.setAttribute("d", "M14.0557 0.416504L7.99984 6.47234L1.944 0.416504L0.416504 1.944L6.47234 7.99984L0.416504 14.0557L1.944 15.5832L7.99984 9.52734L14.0557 15.5832L15.5832 14.0557L9.52734 7.99984L15.5832 1.944L14.0557 0.416504Z");
...
export class GnosisDelegation {
  ...
  openModal = async () => {
    ...
    await this.getOptions()
      .then(async (options) => {
        ...
        return Promise.all([this._connectedAddress, ...options].map((add) => this.web3provider.lookupAddress(add))).then((ensNames) => {
          [this.selectedOption, ...options].forEach((option, index) => {
            const newOption = document.createElement('div');
            newOption.setAttribute('id', `ssx-gnosis-modal--option-${option}`);
  ...
  getTags = (): Record<string, Element> => {
    ...
    const modalCounter = document.querySelector('.ssx-gnosis-modal--results-counter');
    const continueBtn = document.querySelector('#ssx-gnosis-modal--continue-btn');
    return {
      modal,
      modalBody,
      modalCounter,
  ...
  connect = async (): Promise<void> => {
    ...
    this.closeModal();
    this._proceed({ siwe: { address: this.selectedOption.replace(/Yourself - /, '') } });
    return;
  };
}
