...
export class SSXConnected implements ISSXConnected {
  ...
  public async ssxServerNonce(params: Record<string, any>): Promise<string> {
    if (this.api) {
      let nonce;
      try {
        nonce = (await this.api.get(this.config.providers?.server?.endpoints?.nonce ?? '/ssx-nonce', { params })).data;
      } catch (error) {
        console.error(error);
        throw error;
      }
  ...
  public async ssxServerLogin(session: SSXClientSession): Promise<any> {
    if (this.api) {
      ...
      try {
        ...
        return this.api.post(this.config.providers?.server?.endpoints?.login ?? '/ssx-login', {
  ...
  async signOut(session: SSXClientSession): Promise<void> {
    if (this.api) {
      try {
        await this.api.post(this.config.providers?.server?.endpoints?.logout ?? '/ssx-logout', { ...session });
      } catch (error) {
        console.error(error);
        throw error;
      }
